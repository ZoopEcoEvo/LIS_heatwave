...
combined_opt_coll = comb_params %>% 
  ...
  geom_point(size = 3) + 
  scale_colour_manual(values = comb_colors) + 
  theme_pubr() + ylab("Optimum (deg C)") + xlab("Collection Temperature (deg c)")
...
combined_opt_diff = comb_params %>% 
  ...
  geom_point(size = 3) + 
  scale_colour_manual(values = comb_colors) + 
  theme_pubr() + ylab("Margin (deg C)") + xlab("Collection Temperature (deg c)")
...
combined_ld_coll = ggplot(combined_tolerance, aes(x = Coll_temp, y = LD50, colour = Month, group = species)) + 
  ...
  scale_colour_manual(values = comb_colors) + 
  theme_pubr() + 
  ylab("Thermal Tolerance (deg C)") + xlab("Collection Temperature (deg c)")

combined_ld_diff = ggplot(combined_tolerance, aes(x = Coll_temp, y = margin, colour = Month, group = species)) +
  geom_smooth(method = "lm", colour = "grey50") + 
  geom_point(size = 3) + 
  scale_colour_manual(values = comb_colors) + 
  theme_pubr() + ylab("Warming Tolerance (deg C)") + xlab("Collection Temperature (deg c)")
...
F0_data = bind_rows(F0_hs_summary, F0_total_summary,F0_rf_summary, F0_size_summary) %>% 
  dplyr::select(trait, variable, difference, bca_ci_low, bca_ci_high, month, duration, trait, generation) %>% 
  mutate("order_code" = paste(trait, duration, sep = "_"),
         "order_number" = case_when(
         ...
         month = fct_relevel(month, "June", "August", "November"),
         variable = fct_relevel(variable, "Total", "Success", "Hatched", "Size"),
...
F0_grid = ggplot(F0_data, aes(x = duration, y = difference, colour = variable, shape = duration)) + 
  facet_grid(. ~ month) + 
  geom_hline(yintercept = 0, colour = "black", size = 0.3) +
  geom_errorbar(aes(ymin = bca_ci_low, ymax = bca_ci_high), width = 0, size = 1, 
...
F1_grid = ggplot(F1_data, aes(x = duration, y = difference, colour = variable, shape = duration, group = trait)) + 
  facet_grid(off_temp ~ month, ) + 
  geom_hline(yintercept = 0, colour = "black", size = 0.3) +
...
F1_data %>% 
  select(month, duration, off_temp, variable, difference, bca_ci_low, bca_ci_high) %>% 
  mutate("ID" = paste(month, off_temp, variable, sep = "_"),
...
duration_effects = F1_data %>% 
  ...
  arrange(month, off_temp, variable) %>% 
  filter(change == "Different")

select_rnorms = duration_effects %>% 
  select(-duration, -difference, -bca_ci_low, -bca_ci_high, -ID, -zero_diff) %>% 
  distinct()
